    ...
    <Drawer open={props.open} onClose={props.onClose} className={GShellClassName}>
      <GShell>
        <TopicsMenuRoot className={classes.root}>
          ...
          <SimpleTreeView>
            {roots.map(root => (
              <TreeItem itemId={root.id} label={root.topic.name} key={`root-${root.id}`}>
                ...
                {props.topics
                  ...
                  .map(child => (<TreeItem key={child.id} itemId={child.id} label={child.name} onClick={(event) => handleTopicChange(child, event)} />))}
              </TreeItem>
            ))}
          </SimpleTreeView>
        </TopicsMenuRoot>
